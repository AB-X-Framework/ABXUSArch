...
public class DataFrame {
    ...
    public DataFrame(InputStream io) throws Exception {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        baos.write(io.read());
        int _rawIn1 = io.read();
        boolean hasMask = _rawIn1 > 127;
        int length;
        if (_rawIn1 < 126) {
            length = _rawIn1 & 127;
        } else {
            ...
            while (--byteCount > 0) {
                int b = io.read();
                length |= (b & 0xFF) << (8 * byteCount);
            }
        }
        if (hasMask) {
            length += 4;
        }
        readExact(length, io, baos);
        rawData = baos.toByteArray();
        fromRawData();
    }

    private void readExact(int size, InputStream in, OutputStream out) throws Exception {
        while (size > 0) {
            out.write(in.read());
        }
    }
    ...
    /**
     * A convenient constructor to create a Binary Data Frame without
     * extensions.
     *
     * @param fin
     * @param opcode
    ...
    public void validate() throws Exception {
        ...
        if (rsv != 0) {
            throw new Exception("The rsv value is not 0: " + rsv);
        }

        if ((opcode > 2 && opcode < 8) || opcode > 10) {
            throw new Exception(String.format("The opcode[%s] is undefined.", opcode));
        }

        if (mask) {
            throw new Exception("A server MUST NOT mask any frames that it sends to the client");
        }

        //    if (mask && (maskKey == null || maskKey.length != 4)) {
        //      throw new Exception(String.format("The mask is set to true but maskKey[%s] is not correct", maskKey));
        //    } else if (!mask && (maskKey != null && maskKey.length > 0)) {
        //      throw new Exception(String.format("The mask is set to false but maskKey[%s] is not correct", maskKey));
        //    }
        if (payloadLength == -1) {
            throw new Exception("The payload length is not correct");
        }
    ...
    private void fromRawData() {
        ...
        if (length < 0 || length > Integer.MAX_VALUE) {
            payloadLength = -1;
            //throw new Exception("The payload length is not correct: " + dataLength);
        } else {
    ...
    public String toString() {
        ...
        sb.append(", ");
        if (opcode == 8) {
            CloseFrames closeFrame = new CloseFrames(this);
